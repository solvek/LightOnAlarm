...
fun LightOnAlarmScreen(modifier: Modifier = Modifier, viewModel: LightOnAlarmViewModel = hiltViewModel()) {
    ...
    val items by produceState<LightOnAlarmUiState>(
    ...
    if (items is Success) {
        LightOnAlarmScreen(
            ...
            onSave = { name -> viewModel.addLightOnAlarm(name) },
            modifier = modifier
        )
    }
}
...
internal fun LightOnAlarmScreen(
    ...
    Column(modifier) {
        var nameLightOnAlarm by remember { mutableStateOf("Compose") }
        Row(
            modifier = Modifier.fillMaxWidth().padding(bottom = 24.dp),
            horizontalArrangement = Arrangement.spacedBy(16.dp)
        ) {
            TextField(
                value = nameLightOnAlarm,
                onValueChange = { nameLightOnAlarm = it }
            )

            Button(modifier = Modifier.width(96.dp), onClick = { onSave(nameLightOnAlarm) }) {
                Text("Save")
            }
        }
        items.forEach {
...
private fun DefaultPreview() {
    MyApplicationTheme {
        LightOnAlarmScreen(listOf("Compose", "Room", "Kotlin"), onSave = {})
    }
}
